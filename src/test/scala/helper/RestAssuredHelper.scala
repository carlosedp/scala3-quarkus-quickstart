...
extension (spec: RequestSpecification)
     ...
     * DSL expression by sending a request without any parameters or headers
     * etc. E.g.
     * {{{
     * Given()
     *   .When(_.get("/x"))
     ...
     * }}}
     * Note that if you need to add parameters, headers, cookies or other
     * request properties use the [[Given()]] method.
     *
     * @see
...
extension (resp: Response)
     ...
     * returns a [[ValidatableResponse]] that lets you chain validation methods
     * or extract values from the response with the [[Extract]] method. Usage
     * example:
     * {{{
     * Given(_.params("firstName", "John")
     *  .When(_.post("/greetXML"))
    ...
    /**
     * A wrapper around [then] that lets you validate the response. This method
     * is an overloaded version of [Then] that returns Unit so it's
     * automatically picked-up by test frameworks like JUnit which expect tests
     * to return Unit. Usage example:
     * {{{
     * Given(_.params("firstName", "John")
     *  .When(_.post("/greetXML"))
...
extension [T](resp: Response)
    /**
     * A wrapper around [ExtractableResponse] that lets you validate the
     ...
     * parameters `firstName=John` and `lastName=Doe` and expect that the
     * response body containing JSON or XML firstName equal to John. The
     * response is then validated and the firstName is extracted from the
     * response. The extracted firstName is then returned. The type of the
     * extracted value is needs to be specified as a type parameter.
     *
     * @return
     *   The extracted value
     */
...
extension [T](resp: ValidatableResponse)
    /**
     * A wrapper around [ValidatableResponse] that lets you validate the
     * response. Usage example:
     * {{{
     * val firstName: String = Given(_.params("firstName", "John")
     *  .When(_.post("/greetXML"))
